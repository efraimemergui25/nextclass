  ...
  <AnimatePresence>
   {isOpen && (
    ...
    >
     ...
     {/* ── Top bar — logo only ── */}
     <div className="relative z-10 flex items-center justify-end px-8 sm:px-12 pt-8 shrink-0">
      <motion.div
       ...
       return (
        <motion.div
         ...
         exit={{
          ...
          transition: { duration: 0.26, delay: (navItems.length - 1 - i) * 0.03, ease: [0.55, 0, 1, 0.45] },
         }}
        ...
        >
         ...
         <Link
          ...
          style={{ padding: 'clamp(6px, 1.1vh, 14px) 3rem', WebkitTapHighlightColor: 'transparent' }}
         >
          <motion.span
           animate={{
     ...
     <motion.div
      ...
      style={{ paddingBottom: 'clamp(24px, 4vh, 44px)', paddingTop: '14px', gap: '10px' }}
     >
      <span style={{ fontSize: '11px', fontWeight: 500, color: 'rgba(0,0,0,0.22)', letterSpacing: '0.04em', fontFamily: SF, direction: 'ltr' }}>
       {phone}
      </span>
      <span style={{ width: '3px', height: '3px', borderRadius: '50%', background: 'rgba(0,0,0,0.18)', display: 'inline-block', flexShrink: 0 }} />
      <span style={{ fontSize: '10.5px', fontWeight: 800, color: 'rgba(0,0,0,0.18)', letterSpacing: '0.18em', fontFamily: SF }}>
       NEXTCLASS
      </span>
     </motion.div>
    </motion.div>
