...
const FloatingConcierge = () => {
    ...
    const pillLabel  = getSetting('concierge_label', 'צריכים התייעצות?');
    const header     = getSetting('concierge_header', 'דברו איתנו');
    const waNumber   = getSetting('whatsapp_number', '972585856356');
    const waLabel    = getSetting('whatsapp_label', 'וואטסאפ');
    const waSub      = getSetting('whatsapp_sub', 'מענה מהיר עד שעה');
    const phone      = getSetting('contact_phone', '058-5856356');
    const phoneLabel = getSetting('phone_label', 'שיחה עם מומחה');
    const phoneSub   = getSetting('phone_sub', "זמין א'–ה', 9:00–18:00");
    const visWa      = isVisible('vis_whatsapp', true);
    const visPhone   = isVisible('vis_phone', true);
    ...
    const whatsappUrl = `https://wa.me/${waNumber}?text=${encodeURIComponent('שלום, אני מעוניין בהתייעצות לגבי מוצרי NextClass')}`;
        ...
        <div ref={ref} className="fixed bottom-6 right-6 z-[250] flex flex-col items-end gap-3">
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        <p className="text-[11px] font-bold text-[#86868B] px-3 pt-1.5 pb-0.5 text-right">
                            {header}
                        </p>
                        ...
                        {visPhone && (
                            ...
                            >
                                ...
                                <div className="text-right flex-1">
                                    <span className="block text-sm font-bold text-[#1D1D1F] leading-tight">{phoneLabel}</span>
                                    <span className="block text-[11px] text-[#86868B]">{phoneSub}</span>
                                </div>
                            </a>
                        )}

                        {visPhone && visWa && <div className="mx-3 border-t border-gray-100" />}
                        ...
                        {visWa && (
                            ...
                            >
                                ...
                                <div className="text-right flex-1">
                                    <span className="block text-sm font-bold text-[#1D1D1F] leading-tight">{waLabel}</span>
                                    <span className="block text-[11px] text-[#86868B]">{waSub}</span>
                                </div>
                            </a>
                        )}
                    </motion.div>
            ...
            >
                <AnimatePresence mode="wait">
                    {isOpen ? (
                        <motion.span key="close" initial={{ opacity: 0, rotate: -90 }} animate={{ opacity: 1, rotate: 0 }} exit={{ opacity: 0, rotate: 90 }} transition={{ duration: 0.2 }} className="shrink-0">
                            <X size={22} className="text-[#1D1D1F]" />
                        </motion.span>
                    ) : (
                        <motion.span key="msg" initial={{ opacity: 0, rotate: 90 }} animate={{ opacity: 1, rotate: 0 }} exit={{ opacity: 0, rotate: -90 }} transition={{ duration: 0.2 }} className="shrink-0">
                ...
                <AnimatePresence>
                    {isHovered && !isOpen && (
                        <motion.span key="label" initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} transition={spring} className="text-sm font-bold text-[#1D1D1F] whitespace-nowrap overflow-hidden">
                            {pillLabel}
                        </motion.span>
                    )}
                </AnimatePresence>
