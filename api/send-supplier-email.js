...
function buildOrderConfirmationEmail(quote, supplier, customNote) {
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#F8FAFF;border-radius:12px;overflow:hidden;border:1px solid #E8EEFF;">
              <tr>
                ...
                <td style="padding:10px 12px;vertical-align:middle;">
                  <div style="font-size:13px;font-weight:700;color:#1D1D1F;margin-bottom:3px;">${item.title || '—'}</div>
                  ${item.sku ? `<div style="font-size:10px;color:#AEAEB2;font-weight:600;">SKU: ${item.sku}</div>` : ''}
                  ${item.category ? `<div style="display:inline-block;font-size:10px;font-weight:700;color:#5856D6;background:#F0EFFF;padding:2px 7px;border-radius:50px;margin-top:2px;">${item.category}</div>` : ''}
                </td>
                <td style="padding:10px 14px;vertical-align:middle;text-align:left;white-space:nowrap;">
    ...
    const itemsSection = quote.items?.length ? `
      <div style="margin-bottom:28px;">
        <div style="font-size:11px;font-weight:800;color:#6E6E73;margin-bottom:10px;letter-spacing:0.05em;">פירוט הפריטים להזמנה</div>
        <table width="100%" cellpadding="0" cellspacing="0">${itemRows}</table>
      </div>` : '';

    const totalSection = subtotal ? `
      <div style="padding:16px 20px;background:linear-gradient(135deg,#F0EFFF,#F8F7FF);border-radius:14px;border:1.5px solid rgba(88,86,214,0.2);margin-bottom:28px;text-align:center;">
        <div style="font-size:11px;font-weight:700;color:#8E8E93;margin-bottom:4px;">סה״כ הזמנה מוסכם</div>
        <div style="font-size:28px;font-weight:900;color:#5856D6;">₪${subtotal.toLocaleString()}</div>
      </div>` : '';
    ...
    const trackingRequestSection = `
      <div style="border-right:4px solid #5856D6;background:#F5F4FF;border-radius:0 12px 12px 0;padding:18px 20px;margin-bottom:0;">
        <div style="font-size:13px;font-weight:800;color:#1D1D1F;margin-bottom:12px;">📩 אנא אשרו ושלחו בחזרה:</div>
        <div style="display:flex;flex-direction:column;gap:10px;">
          <div style="display:flex;align-items:flex-start;gap:8px;">
            <span style="background:#5856D6;color:#fff;font-size:10px;font-weight:900;width:18px;height:18px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px;">1</span>
            <span style="font-size:13px;color:#3D3D3D;line-height:1.6;"><strong>אישור קבלת ההזמנה</strong> ומספר הזמנה מצדכם</span>
          </div>
          <div style="display:flex;align-items:flex-start;gap:8px;">
            <span style="background:#5856D6;color:#fff;font-size:10px;font-weight:900;width:18px;height:18px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px;">2</span>
            <span style="font-size:13px;color:#3D3D3D;line-height:1.6;"><strong>מספר מעקב משלוח</strong> + שם חברת השליחות</span>
          </div>
          <div style="display:flex;align-items:flex-start;gap:8px;">
            <span style="background:#5856D6;color:#fff;font-size:10px;font-weight:900;width:18px;height:18px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px;">3</span>
            <span style="font-size:13px;color:#3D3D3D;line-height:1.6;"><strong>תאריך אספקה משוער</strong> — להעברת ללקוח הסופי</span>
          </div>
        </div>
        <div style="font-size:12px;color:#6E6E73;border-top:1px solid rgba(88,86,214,0.12);padding-top:10px;margin-top:14px;">לכל שאלה: <strong>${BIZ_PHONE}</strong> · <a href="mailto:${BIZ_EMAIL}" style="color:#5856D6;text-decoration:none;font-weight:700;">${BIZ_EMAIL}</a></div>
      </div>`;

    const body = `
      <p style="margin:0 0 24px;font-size:15px;color:#1D1D1F;line-height:1.75;">${greeting}<br/><br/>
        לאחר קבלת הצעת המחיר שלכם, אנו שמחים לאשר ולבצע את ההזמנה הבאה.<br/>
      ...
      ${deliverySection}
      ${divider()}
      ${trackingRequestSection}
    `;

    const preheader = `אישור הזמנה ${quote.id} מ-NextClass — ממתין לאישורכם ומספר מעקב`;

    return `<!DOCTYPE html>
<html lang="he" dir="rtl">
<head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>הזמנה ${quote.id} — NextClass</title></head>
<body style="margin:0;padding:0;background:#F2F0FF;font-family:${FONT};-webkit-font-smoothing:antialiased;direction:rtl;">

<div style="display:none;max-height:0;overflow:hidden;">${preheader}&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;</div>

<table width="100%" cellpadding="0" cellspacing="0" style="background:#F2F0FF;min-height:100vh;">
<tr><td align="center" style="padding:40px 16px;">
<table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
  ...
  <tr><td style="padding-bottom:16px;text-align:center;">
    <div style="display:inline-block;background:#5856D6;color:#fff;font-size:10px;font-weight:800;padding:5px 16px;border-radius:50px;letter-spacing:0.1em;">
      ✅ הזמנה רשמית מאושרת
    </div>
  </td></tr>
  ...
  <tr><td style="background:#FFFFFF;border-radius:24px;box-shadow:0 1px 4px rgba(0,0,0,0.06),0 8px 32px rgba(0,0,0,0.07);overflow:hidden;">
    ...
    <div style="height:4px;background:linear-gradient(90deg,#5856D6 0%,#007AFF 100%);border-radius:24px 24px 0 0;"></div>

    <div style="background:linear-gradient(180deg,#EEF0FF 0%,#F5F4FF 50%,#FAFAFF 100%);padding:36px 40px 30px;text-align:center;">
      <table cellpadding="0" cellspacing="0" style="margin:0 auto 18px;">
        <tr>
          <td style="width:68px;height:68px;border-radius:50%;background:linear-gradient(135deg,#5856D6,#007AFF);text-align:center;vertical-align:middle;box-shadow:0 8px 28px rgba(88,86,214,0.35);">
            <span style="font-size:30px;color:#fff;line-height:68px;font-weight:900;">📋</span>
          </td>
        </tr>
      </table>
      <div style="font-size:24px;font-weight:900;color:#1D1D1F;letter-spacing:-0.5px;margin-bottom:5px;">אישור הזמנה — NextClass</div>
      <div style="font-size:14px;color:#6E6E73;line-height:1.5;">הזמנה רשמית לביצוע — ממתינים לאישורכם ולמספר מעקב</div>
      <div style="margin-top:18px;display:inline-block;background:#FFFFFF;border:1.5px solid #5856D6;border-radius:50px;padding:8px 24px;">
        <span style="font-size:12px;color:#6E6E73;font-weight:600;">מספר הזמנה</span>
        <span style="font-size:14px;color:#5856D6;font-weight:900;margin-right:10px;">${quote.id}</span>
      </div>
      ${so.orderNumber ? `<div style="margin-top:8px;display:inline-block;background:#F0EFFF;border:1px solid rgba(88,86,214,0.2);border-radius:50px;padding:5px 18px;">
        <span style="font-size:11px;color:#6E6E73;font-weight:600;">מספר הזמנה שלכם</span>
        <span style="font-size:12px;color:#5856D6;font-weight:800;margin-right:8px;">${so.orderNumber}</span>
      </div>` : ''}
    </div>

    <div style="padding:32px 40px;">${body}</div>

    <div style="background:#F2F0FF;border-top:1px solid #E4E2FF;padding:22px 40px;text-align:center;">
      <div style="font-size:13px;font-weight:700;color:#1D1D1F;margin-bottom:5px;">NextClass — ציוד טכנולוגי לחינוך ישראל</div>
      <div style="font-size:12px;color:#6E6E73;">
        <a href="tel:${BIZ_PHONE.replace(/\D/g,'')}" style="color:#5856D6;text-decoration:none;font-weight:600;">${BIZ_PHONE}</a>
        &nbsp;·&nbsp;
        <a href="mailto:${BIZ_EMAIL}" style="color:#5856D6;text-decoration:none;font-weight:600;">${BIZ_EMAIL}</a>
        &nbsp;·&nbsp;
